    ...
    public void updateConnections() {
        ...
        if (this.controller.isInCurrentView()) {
            ...
            for (int i = 0; i < toRemove.size(); i++) {
                logger.info("Removing connection toRemove.get(i): {} ", toRemove.get(i));
...
//        logger.info("inside send with useMAC :{}", useMAC);
...
//                logger.info("Interruption while inserting message into inqueue", ex);
    ...
    public void joinViewReceived() {
        ...
        for (int i = 0; i < pendingConn.size(); i++) {
            ...
            try {
                logger.info("Join View Received");
                establishConnection(pc.s, pc.remoteId);
            } catch (Exception e) {
                logger.info("Failed to estabilish connection to " + pc.remoteId,e);
            }
        }
    ...
    public void run() {
        while (doWork) {
            try {
                ...
                if (!this.controller.isInCurrentView() &&
                     remoteId<1000){//(this.controller.getStaticConf().getTTPId() != remoteId) ) {
                    waitViewLock.lock();
                    logger.debug("cbqwbub Trying establish connection with Replica: {}, {}, {}", remoteId,newSocket, serverSocketSSLTLS.getInetAddress());
				...
				logger.info("Server socket timed out, retrying");
			} catch (SSLHandshakeException sslex) {
                logger.info("SSLHandshakeException");

                sslex.printStackTrace();
			} catch (IOException ex) {
				logger.info("Problem during thread execution", ex);
			}
        }
