...
public class Synchronizer {
        ...
        if (lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            try { // serialize content to send in STOP message
                ...
                if (messages != null && messages.size() > 0) {
                    ...
                    logger.info("Strange... did not include any request in my STOP message for regency " + regency);
                }
    ...
    private void addSTOPedRequestsToClientManager() {
        ...
        if (messagesFromSTOP != null) {
            ...
            logger.info("Adding to client manager the requests contained in STOP messages");
    ...
    private void startSynchronization(int nextReg) {
        ...
        if (condition && lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            logger.info("Initialize synch phase");
            requestsTimer.Enabled(false);
            requestsTimer.stopTimer();
        ...
        if (condition && lcManager.getNextReg() > lcManager.getLastReg())
        {
            ...
            logger.info("Installing regency " + lcManager.getNextReg());
            ...
            if (leader != this.controller.getStaticConf().getProcessId()) {
                ...
                { // serialize content of the STOPDATA message
                    ...
                    } else
                    {
                        ...
                        cons.createEpoch(regency, controller);
                        //Logger.println("(Synchronizer.startSynchronization) incrementing ets of consensus " + cons.getId() + " to " + ets);
                        logger.info("Incrementing ets of consensus " + cons.getId() + " to " + regency);
		...
		//TODO: Turn on timeout again?
                ...
                logger.info("Checking if there are out of context SYNC for regency " + regency);
                ...
                logger.info("I'm the leader for this new regency");
                ...
                if (stopdatas.size() > 0) {
                    logger.info("Processing " + stopdatas.size() + " out of context STOPDATAs for regency " + regency);
                } else {
                    logger.info("No out of context STOPDATAs for regency " + regency);
                }
