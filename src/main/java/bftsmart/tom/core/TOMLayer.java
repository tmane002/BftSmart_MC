...
//        return dt.getLastCID();
    ...
    public void requestReceived(TOMMessage msg, boolean fromClient) {
        ...
        if (readOnly) {
            ...
            logger.info("Received TOMMessage from client " + msg.getSender() + " with sequence number " + msg.getSequence() + " for session " + msg.getSession());
            ...
            if (clientsManager.requestReceived(msg, fromClient, communication)) {
                ...
                logger.info("Tejas: The received TOMMessage " + msg + " was discarded.");
            }
        }
    }
    ...
    public void run() {
        ...
        while (doWork) {
            ...
            logger.info("iAmLeader awaitUninterruptibly signalled to proceed ");

            if (!doWork) break;
            logger.info("did not break due to doWork ");
    ...
    public void setNoExec() {
        logger.info("Modifying inExec from " + this.inExecution + " to " + -1);
...
//    private void haveMessagesOtherClusters() {
//        messagesLock.lock();
//        haveMessages.signal();
//        messagesLock.unlock();
//    }
    ...
    public void signalWaitingForLCMessageOCReply() {

        dt.DTsignalWaitingForLCMessageOCReply();
    }
    ...
    public void signalReconfigConfirmationNewNode() {
        dt.signalReconfigConfirmationNewNode();
    }

    public void StartLeaderChange(SystemMessage sm) {
        int[] myself = new int[1];
        myself[0] = controller.getStaticConf().getProcessId();

        if (((SMMessage) sm).getCID() > LastLeaderChangeId)
        {
