...
//        if (2>1)
    ...
    public void receiveMessages(int consId[], int regencies[], int leaders[],
...
//        logger.info("Tejas: ----> completed inside receiveMessages");
    }
    ...
    public void receiveMessages(int consId[], int regencies[], int leaders[],
                                CertifiedDecision[] cDecs, TOMMessage[][] requests, ArrayList<OtherClusterMessage> ocmArray) throws IOException, ClassNotFoundException {
        int numRequests = 0;
        int consensusCount = 0;
        List<TOMMessage> toBatch = new ArrayList<>();
        List<MessageContext> msgCtxts = new ArrayList<>();
        ...
        for(OtherClusterMessage ocm:ocmArray)
        {
            for (TOMMessage[] msg:ocm.getOcmd().requests)
            {

                for(TOMMessage tmsg:msg)
                {
//                    logger.info("tmsg data is {}, {}, {}", tmsg.consensusStartTime, tmsg.timestamp, tmsg.seed);
                }
            }
        }

        ArrayList<TOMMessage[]> requestsCombined = new ArrayList<TOMMessage[]>();

        for (TOMMessage[] requestsFromConsensus : requests)
        {
            requestsCombined.add(requestsFromConsensus);
        }
//        for(OtherClusterMessage ocm:ocmArray)
//        {
//            for (TOMMessage[] msg:ocm.getOcmd().requests)
...
//        }
        ...
        ArrayList<Integer> consIdCombined = new ArrayList<Integer>();

        for (int i : consId)
        {
            consIdCombined.add(i);
        }
//        for(OtherClusterMessage ocm:ocmArray)
//        {
//            for (int i :ocm.getOcmd().consId)
//            {
//                    consIdCombined.add(i);
//
//            }
//        }
        ...
        for (TOMMessage[] requestsFromConsensus : requestsCombined) {
            ...
            for (TOMMessage request : requestsFromConsensus) {
                ...
                logger.debug("Processing TOMMessage from client " + request.getSender() + " with sequence number "
                        + request.getSequence() + " for session " + request.getSession() + " decided in consensus "
                        + consIdCombined.get(consensusCount));
...
//            logger.info("Supposed to NOT RUN since NOT IN VIEW");
