...
public class DefaultApplicationState implements ApplicationState {
    ...
    private HashMap<Integer, Integer> stateHmap = new HashMap<Integer, Integer>();

    LinkedBlockingQueue<OtherClusterMessage> DecidedOtherClusters;
    ...
    byte[] state, byte[] stateHash, int pid, HashMap<Integer, Integer> hmp) {
...
//        logger.info("------->>>>>>>>>> getLastCID(), ci are {}, {}",getLastCID(),ci);
        if (ci != null && ci.msgCtx[0].getProof() != null) { // do I have a proof for the consensus?
//        if (2>1) { // do I have a proof for the consensus?

                Set<ConsensusMessage> proof = ci.msgCtx[0].getProof();
//            Set<ConsensusMessage> d = new TreeSet<>();
...
//        {
//            byte[] t1 = new byte[2];
//            Set<ConsensusMessage> t2 = new HashSet<ConsensusMessage>();
//
//            return new CertifiedDecision(pid, getLastCID(), t1, t2); // there was no proof for the consensus
//
//        }
...
//        logger.info("messageBatches, cid, lastCheckpointCID, lastCID are {}, {}, {}, {}",
//                messageBatches, cid, lastCheckpointCID, lastCID);

        if (messageBatches != null && cid >= lastCheckpointCID && cid <= lastCID) {
//            logger.info("cid, lastCheckpointCID are {}, {}", cid, lastCheckpointCID);
    ...
    @Override
    public HashMap<Integer, Integer> getStateHmap()
    {
        return stateHmap;
    }
    ...
    @Override
    public void setStateHmap(HashMap<Integer, Integer> hmp)
    {
        stateHmap = hmp;
    }
