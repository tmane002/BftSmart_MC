...
public abstract class StateManager {
    ...
    protected boolean enoughProofs(int cid, LCManager lc) {
        ...
        for (CertifiedDecision cDec : senderProofs.values()) {
            logger.info("Tejas-------> cDec, cid are {} and {}",cid, cDec);
...
//            }
        ...
        logger.info("Tejas-------> counter, SVController.getQuorum() are {} and {}",counter, SVController.getQuorum());
        return result;
    }
        ...
        while (isInitializing) {
           ...
           queryID++;
            ...
            try {
                ...
                logger.info("Interruption during sleep",e);
            }
        }
    }
...
//        int lastConsensusId = tomLayer.getLastExec();
        ...
        int lastConsensusId = tomLayer.getStateManager().getLastCID();


//        CertifiedDecision thisDec = dt.getRecoverer().getState(lastConsensusId, true).getCertifiedDecision(SVController);


//        while(thisDec ==null && lastConsensusId > 0)
//        {
//            lastConsensusId = lastConsensusId-1;
//            thisDec = dt.getRecoverer().getState(lastConsensusId, true).getCertifiedDecision(SVController);
//        }
...
//        DefaultApplicationState state = new DefaultApplicationState(null, -1, lastConsensusId, null, null, -1);
        ...
        DefaultApplicationState state = new DefaultApplicationState(null, -1,
                lastConsensusId, null, null, -1, dt.getHmap());
